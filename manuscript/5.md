...
In this case, for example, most servers would already respond to a request with a `Last-Modified` header. (The other two headers seem non-standard, but could be provided, too, if necessary.) The server the example site is on is no exception:
